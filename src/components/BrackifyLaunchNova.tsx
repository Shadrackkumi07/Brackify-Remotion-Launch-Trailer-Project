...
import {
  DEFAULT_SPLINE_SCENE_URL,
  SplineImportedModel,
} from "./spline/SplineImportedModel";
...
export const BRACKIFY_NOVA_DURATION = 42 * BRACKIFY_NOVA_FPS;
...
const HeroBracketVisual: React.FC<{ duration: number; variant: Variant }> = ({
  ...
  const frame = useCurrentFrame();
  const SPLINE_HERO_FRAMES = 180;
  const localFrame = Math.max(0, frame - SPLINE_HERO_FRAMES);
  const localDuration = Math.max(1, duration - SPLINE_HERO_FRAMES);
  ...
  const bracketOpacity = interpolate(frame, [SPLINE_HERO_FRAMES - 20, SPLINE_HERO_FRAMES + 20], [0, 1], clamp);

  return (
    <AbsoluteFill>
      {frame < SPLINE_HERO_FRAMES ? (
        <ThreeCanvas width={variant === "mobile" ? 1080 : 1920} height={variant === "mobile" ? 1920 : 1080}>
          <ambientLight intensity={0.82} />
          <pointLight position={[12, 8, -6]} intensity={1.6} color="#a7e6ff" />
          <pointLight position={[-10, -4, 8]} intensity={1.3} color="#4de2c5" />
          <pointLight position={[0, -8, -12]} intensity={0.85} color="#ff7a45" />
          <Suspense fallback={null}>
            <SplineImportedModel
              sceneUrl={DEFAULT_SPLINE_SCENE_URL || undefined}
              variant={variant}
            />
          </Suspense>
        </ThreeCanvas>
    ...
    >
      ...
      <div
        style={{
          ...
          opacity: frame > 120 && frame < BRACKIFY_NOVA_DURATION - 240 ? 0.7 : 0,
        }}
      >
        <Img
          src={staticFile("logo.png")}
